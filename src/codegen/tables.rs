...
/// 事件处理器映射表：(camelCase, snake_case)
///
/// snake_case 同时作为 GPUI 方法名使用（两者始终相同）
pub(crate) const EVENT_HANDLERS: &[(&str, &str)] = &[
    ("onClick", "on_click"),
...
/// 颜色映射表 — 完整 Tailwind 色板（按名称字母序排列，支持二分查找）
...
/// 查找颜色值（二分查找，O(log n)）
///
/// COLOR_MAP 已按名称字母序排列，使用二分查找替代线性扫描。
/// 242 条目：最多 8 次比较即可命中或确认不存在。
pub(crate) fn lookup_color(name: &str) -> Option<u32> {
    COLOR_MAP
        .binary_search_by_key(&name, |(color_name, _)| color_name)
        .ok()
        .map(|idx| COLOR_MAP[idx].1)
}
